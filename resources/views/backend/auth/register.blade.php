<?php $general_setting = DB::table('general_settings')->find(1); 
    $accounts = DB::table('pos_account')->select('ACCNT_ID', 'ACCNT_NAME')->get();
  ...
  <body>
    ...
    <select required name="role_id" id="role-id" class="form-control">
        ...
        @foreach($lims_role_list as $role)
            @if($role->id == 1) <!-- Only display Admin (role_id == 5) -->
...
<div class="form-group-material">
    <select required name="account_id" id="account-id" class="form-control">
        <option value="">Select Account*</option>
        @foreach ($accounts as $account)
                      <option value="{{$account->ACCNT_ID}}">{{$account->ACCNT_NAME}}</option>
                  @endforeach
    </select>
</div>
