<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\Department;
use Illuminate\Validation\Rule;
use Spatie\Permission\Models\Role;
use Spatie\Permission\Models\Permission;
use Auth;

class DepartmentController extends Controller
{
    public function index()
{
    $role = Role::find(Auth::user()->role_id);
    if($role->hasPermissionTo('department')) {
        // Get the logged-in user's pos_accnt_id
        $pos_accnt_id = Auth::user()->pos_accnt_id;
        
        // Filter departments by the user's pos_accnt_id
        $lims_department_all = Department::where('is_active', true)
                                        ->where('pos_accnt_id', $pos_accnt_id)
                                        ->get();
                                        
        return view('backend.department.index', compact('lims_department_all'));
    }
    else
        return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
}

    public function store(Request $request)
{
    $this->validate($request, [
        'name' => [
            'max:255',
                Rule::unique('departments')->where(function ($query) {
                return $query->where('is_active', 1)
                            ->where('pos_accnt_id', Auth::user()->pos_accnt_id);
            }),
        ],
    ]);
    
    $data = $request->all();
    $data['is_active'] = true;
    // Add the admin's pos_accnt_id to the data array
    $data['pos_accnt_id'] = Auth::user()->pos_accnt_id;
    
    Department::create($data);
    return redirect('departments')->with('message', 'Department created successfully');
}
   
   public function update(Request $request, $id)
{
    $this->validate($request,[
        'name' => [
            'max:255',
            Rule::unique('departments')->ignore($request->department_id)->where(function ($query) {
                return $query->where('is_active', 1)
                            ->where('pos_accnt_id', Auth::user()->pos_accnt_id);
            }),
        ],
    ]);
    
    $data = $request->all();
    $lims_department_data = Department::find($data['department_id']);
    
    // Check if the department belongs to the current user's pos_accnt_id
    if ($lims_department_data->pos_accnt_id != Auth::user()->pos_accnt_id) {
        return redirect('departments')->with('not_permitted', 'You are not authorized to update this department');
    }
    
    $lims_department_data->update($data);
    return redirect('departments')->with('message', 'Department updated successfully');
}

    

 public function deleteBySelection(Request $request)
{
    $department_id = $request['departmentIdArray'];
    $user_pos_accnt_id = Auth::user()->pos_accnt_id;
    
    foreach ($department_id as $id) {
        $lims_department_data = Department::find($id);
        
        // Only delete if department belongs to the current user's pos_accnt_id
        if ($lims_department_data->pos_accnt_id == $user_pos_accnt_id) {
            $lims_department_data->is_active = false;
            $lims_department_data->save();
        }
    }
    
    return 'Department deleted successfully!';
}

public function destroy($id)
{
    $lims_department_data = Department::find($id);
    
    // Check if the department belongs to the current user's pos_accnt_id
    if ($lims_department_data->pos_accnt_id != Auth::user()->pos_accnt_id) {
        return redirect('departments')->with('not_permitted', 'You are not authorized to delete this department');
    }
    
    $lims_department_data->is_active = false;
    $lims_department_data->save();
    return redirect('departments')->with('message', 'Department deleted successfully');
}
}
