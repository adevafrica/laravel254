...
class HomeController extends Controller
{
    ...
    public function landlordDocumentation()
    {
        $general_setting =  Cache::remember('general_setting', 60*60*24*365, function () {
            return DB::table('general_settings')->latest()->first();
        });
        return view('landlord.documentation', compact('general_setting'));
    }
  ...
  public function dashboard()
{
    ...
    if(Auth::user()->role_id == 5) {
        ...
        $lims_payment_data = DB::table('payments')
                       ...
                       ->where('customer_id', $customer->id)
                       ->where('payments.pos_accnt_id', $pos_accnt_id)
        ...
        $lims_reward_point_setting_data = RewardPointSetting::select('per_point_amount')
                                        ->where('pos_accnt_id', $pos_accnt_id)
                                        ->latest()
                                        ->first();
    ...
    while($start < $end)
    {
        ...
        if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own') {
            $recieved_amount = DB::table('payments')
            ...
            $return_amount = Returns::whereDate('created_at', '>=' , $start_date)
                        ->where('pos_accnt_id', $pos_accnt_id)
                        ->whereDate('created_at', '<=' , $end_date)
                        ->where('user_id', Auth::id())
                        ->sum('grand_total');
                        
            $purchase_return_amount = ReturnPurchase::whereDate('created_at', '>=' , $start_date)
                                    ->where('pos_accnt_id', $pos_accnt_id)
                                    ->whereDate('created_at', '<=' , $end_date)
                                    ->where('user_id', Auth::id())
                                    ->sum('grand_total');
                                    
            $expense_amount = Expense::whereDate('created_at', '>=' , $start_date)
                            ->where('pos_accnt_id', $pos_accnt_id)
                            ->whereDate('created_at', '<=' , $end_date)
                            ->where('user_id', Auth::id())
                            ->sum('amount');
                            
            $payroll_amount = Payroll::whereDate('created_at', '>=' , $start_date)
                            ->where('pos_accnt_id', $pos_accnt_id)
                            ->whereDate('created_at', '<=' , $end_date)
                            ->where('user_id', Auth::id())
                            ->sum('amount');
   ...
   public function yearlyBestSellingPrice()
{
    ...
    // Get current logged in user's account ID
    $pos_accnt_id = auth()->user()->pos_accnt_id;
    ...
    $yearly_best_selling_price = Product_Sale::join('products', 'products.id', '=', 'product_sales.product_id')
        ...
        // Filter by the current user's account ID
        ->where('product_sales.pos_accnt_id', $pos_accnt_id)
    ...
    public function monthlyBestSellingQty()
{
    ...
    $best_selling_qty = Product_Sale::join('products', 'products.id', '=', 'product_sales.product_id')
        ...
        // Filter by pos_accnt_id to implement multi-tenancy
        ->where('product_sales.pos_accnt_id', $pos_accnt_id)
        ->where('products.pos_accnt_id', $pos_accnt_id)
        ->groupBy('products.code')
        ->orderBy('sold_qty', 'desc')
...
{
    // Get the current user's pos_accnt_id for tenant filtering
    $pos_accnt_id = Auth::user()->pos_accnt_id;

    // Base query with tenant filtering
    $query = Sale::join('customers', 'customers.id', '=', 'sales.customer_id')
                ->where('sales.pos_accnt_id', $pos_accnt_id)
                ->select('sales.id', 'sales.reference_no', 'sales.sale_status', 'sales.created_at', 'sales.grand_total', 'customers.name')
                ->orderBy('id', 'desc')
                ->take(5);
    ...
    if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own') {
        ...
        $query->addSelect('sales.user_id'); // Add user_id to selection
    }

    $recent_sale = $query->get();
...
public function recentPurchase()
{
    ...
    $query = Purchase::leftJoin('suppliers', 'suppliers.id', '=', 'purchases.supplier_id')
        ->select('purchases.id', 'purchases.reference_no', 'purchases.status', 
                'purchases.created_at', 'purchases.grand_total', 'suppliers.name')
        ->where('purchases.pos_accnt_id', $user_pos_accnt_id) // Multi-tenancy filter
        ->orderBy('purchases.id', 'desc')
        ->take(5);
    
    // Additional filtering based on staff_access setting
    if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own')
    {
        $query->where('purchases.user_id', Auth::id());
    }
    
    $recent_purchase = $query->get();
...
public function recentQuotation()
{
    ...
    $query = Quotation::join('customers', 'customers.id', '=', 'quotations.customer_id')
        ->select('quotations.id', 'quotations.reference_no', 'quotations.quotation_status', 
                'quotations.created_at', 'quotations.grand_total', 'customers.name')
        ->where('quotations.pos_accnt_id', $user_pos_accnt_id) // Multi-tenancy filter
    ...
    if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own')
    {
        ...
        // Add user_id to select statement when needed for staff filtering
        $query->addSelect('quotations.user_id');
    }
    
    $recent_quotation = $query->get();
   ...
   public function recentPayment()
{
    ...
    if(Auth::user()->role_id > 2 && cache()->get('general_setting')->staff_access == 'own')
    {
        // Staff can only see their own payments that also match their pos_accnt_id
        $recent_payment = Payment::select('id','payment_reference','amount','paying_method','created_at','user_id')
                            ->where('user_id', Auth::id())
                            ->where('pos_accnt_id', $userPosAccntId)
                            ->orderBy('id', 'desc')
                            ->take(5)
                            ->get();
    }
    ...
    {
        // Admin/Super users can see all payments but only for their pos_accnt_id
        $recent_payment = Payment::select('id','payment_reference','amount','paying_method','created_at')
                            ->where('pos_accnt_id', $userPosAccntId)
                            ->orderBy('id', 'desc')
                            ->take(5)
                            ->get();
