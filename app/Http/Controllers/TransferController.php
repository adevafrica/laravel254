   ...
   public function index(Request $request)
{
    ...
    if($role->hasPermissionTo('transfers-index')) {
        ...
        // Filter warehouses based on the logged-in admin's pos_accnt_id
...
public function transferData(Request $request)
{
    ...
    // Get warehouses associated with the logged-in user's pos_accnt_id
    $userWarehouses = \DB::table('warehouses')->where('pos_accnt_id', $user_pos_accnt_id)->pluck('id')->toArray();
    
    // Start building the query
    $q = Transfer::whereDate('created_at', '>=' ,$request->input('starting_date'))
                 ->whereDate('created_at', '<=' ,$request->input('ending_date'));
    
    // Apply multi-tenancy filter - only show records matching the user's pos_accnt_id
    $q = $q->where('transfers.pos_accnt_id', $user_pos_accnt_id);
    
    // Filter transfers to only include transfers where both from and to warehouses belong to user's pos_accnt_id
    $q = $q->whereIn('from_warehouse_id', $userWarehouses)
           ->whereIn('to_warehouse_id', $userWarehouses);
    
    // Apply existing permission-based filters
    if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
        $q = $q->where('user_id', Auth::id());
    elseif(Auth::user()->role_id > 2 && config('staff_access') == 'warehouse')
        $q = $q->where(function($query) {
            $query->where('from_warehouse_id', Auth::user()->warehouse_id)
    ...
    if(empty($request->input('search.value'))) {
        ...
        $q = Transfer::with('fromWarehouse', 'toWarehouse', 'user')
            ->where('transfers.pos_accnt_id', $user_pos_accnt_id)
            ->whereIn('from_warehouse_id', $userWarehouses)
            ->whereIn('to_warehouse_id', $userWarehouses)
        ...
        // Base search query
        $q = Transfer::where('transfers.pos_accnt_id', $user_pos_accnt_id)
            ->whereIn('from_warehouse_id', $userWarehouses)
            ->whereIn('to_warehouse_id', $userWarehouses);
            
        // Try to parse date from search or use reference_no search
        if(strtotime(str_replace('/', '-', $search))) {
            $q = $q->whereDate('transfers.created_at', '=', date('Y-m-d', strtotime(str_replace('/', '-', $search))));
        } else {
            $q = $q->where('reference_no', 'LIKE', "%{$search}%");
        }
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $transfers = $q->select('transfers.*')
                ...
                ->where(function($query) use ($search, $user_pos_accnt_id, $userWarehouses) {
                    $query->where('transfers.user_id', Auth::id())
                          ->where('transfers.pos_accnt_id', $user_pos_accnt_id)
                          ->whereIn('from_warehouse_id', $userWarehouses)
                          ->whereIn('to_warehouse_id', $userWarehouses);
                })
        ...
        elseif(Auth::user()->role_id > 2 && config('staff_access') == 'warehouse') {
            $transfers = $q->select('transfers.*')
                ->with('fromWarehouse', 'toWarehouse', 'user')
                ->where(function($query) use ($user_pos_accnt_id, $userWarehouses) {
                    $query->where(function($subquery) {
                            $subquery->where('from_warehouse_id', Auth::user()->warehouse_id)
                                    ->orWhere('to_warehouse_id', Auth::user()->warehouse_id);
                        })
                        ->where('transfers.pos_accnt_id', $user_pos_accnt_id)
                        ->whereIn('from_warehouse_id', $userWarehouses)
   ...
   public function create()
{
    ...
    if($role->hasPermissionTo('transfers-add')){
        ...
        // Filter warehouses by pos_accnt_id to implement multi-tenancy
    ...
    public function limsProductSearchOld(Request $request)
    {
    ...
    public function store(Request $request)
    {
        ...
        // Fetch the pos_accnt_id of the logged-in admin
    $posAccntId = Auth::user()->pos_accnt_id;

    // Get all the other request data, excluding the document
    $data = $request->except('document');

    // Add the pos_accnt_id to the data being saved
    $data['user_id'] = Auth::id();
    $data['reference_no'] = 'tr-' . date("Ymd") . '-' . date("his");
    $data['pos_accnt_id'] = $posAccntId;  // Add the pos_accnt_id
    ...
    public function getProductTransferData($id)
{
    ...
    // Modify the query to filter transfers based on pos_accnt_id
    $lims_product_transfer_data = ProductTransfer::where('transfer_id', $id)
        ->where('pos_accnt_id', $posAccntId)  // Add this line to filter based on pos_accnt_id
        ->get();
