    ...
    public function index(Request $request)
{
    ...
    $warehouse_id = $request->input('warehouse_id');
    $admin_pos_accnt_id = Auth::user()->pos_accnt_id; // Get the logged-in admin's pos_accnt_id

    // Base query with pos_accnt_id filter
    $baseQuery = Returns::where('pos_accnt_id', $admin_pos_accnt_id)
                ->whereDate('created_at', '>=' ,$request->input('starting_date'))
                ->whereDate('created_at', '<=' ,$request->input('ending_date'));

    // Total data count with various filtering conditions
    if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
        $totalData = $baseQuery->where('user_id', Auth::id())->count();
    elseif(Auth::user()->role_id > 2 && config('staff_access') == 'warehouse')
        $totalData = $baseQuery->where('warehouse_id', Auth::user()->warehouse_id)->count();
    elseif($warehouse_id != 0)
        $totalData = $baseQuery->where('warehouse_id', $warehouse_id)->count();
    else
    ...
    if(empty($request->input('search.value'))) {
        $q = Returns::with('biller', 'customer', 'warehouse', 'user')
            ->where('pos_accnt_id', $admin_pos_accnt_id) // Apply pos_accnt_id filter
            ->whereDate('created_at', '>=' ,$request->input('starting_date'))
            ->whereDate('created_at', '<=' ,$request->input('ending_date'))
            ->offset($start)
            ->limit($limit)
    ...
    else
    {
        ...
        $q = Returns::join('customers', 'returns.customer_id', '=', 'customers.id')
            ...
            ->where('returns.pos_accnt_id', $admin_pos_accnt_id) // Apply pos_accnt_id filter
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $returnss =  $q->select('returns.*')
                        ...
                        ->orwhere(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('returns.reference_no', 'LIKE', "%{$search}%")
                                  ->where('returns.user_id', Auth::id())
                                  ->where('returns.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orwhere(function($query) use ($search, $admin_pos_accnt_id) {
                        ...
                        ->orwhere(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('customers.phone_number', 'LIKE', "%{$search}%")
                                  ->where('returns.user_id', Auth::id())
                                  ->where('returns.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orwhere(function($query) use ($search, $admin_pos_accnt_id) {
        ...
        elseif(Auth::user()->role_id > 2 && config('staff_access') == 'warehouse') {
            $returnss =  $q->select('returns.*')
                        ->with('biller', 'customer', 'warehouse', 'user')
                        ->where(function($query) use ($admin_pos_accnt_id) {
                            $query->where('returns.warehouse_id', Auth::user()->warehouse_id)
                                  ->where('returns.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orwhere(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('returns.reference_no', 'LIKE', "%{$search}%")
                                  ->where('returns.warehouse_id', Auth::user()->warehouse_id)
                                  ->where('returns.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orwhere(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('customers.name', 'LIKE', "%{$search}%")
            ...
            $totalFiltered = $q->where('returns.warehouse_id', Auth::user()->warehouse_id)
                            ->orwhere(function($query) use ($search, $admin_pos_accnt_id) {
                                $query->where('returns.reference_no', 'LIKE', "%{$search}%")
                                      ->where('returns.warehouse_id', Auth::user()->warehouse_id)
                                      ->where('returns.pos_accnt_id', $admin_pos_accnt_id);
            ...
            $totalFiltered = $q->where('returns.pos_accnt_id', $admin_pos_accnt_id)
                            ->where(function($query) use ($search) {
                                $query->orwhere('returns.reference_no', 'LIKE', "%{$search}%")
                                      ->orwhere('customers.name', 'LIKE', "%{$search}%")
                                      ->orwhere('customers.phone_number', 'LIKE', "%{$search}%")
