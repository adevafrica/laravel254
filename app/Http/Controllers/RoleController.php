...
class RoleController extends Controller
{
    public function index()
    {
        if(Auth::user()->role_id <= 2) {
            // Filter roles by the current user's pos_accnt_id
            $lims_role_all = Roles::where('is_active', true)
    ...
    {
        // Empty method as per original implementation
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->all();
        // Add the pos_accnt_id from the authenticated user
    ...
    {
        if(Auth::user()->role_id <= 2) {
            // Get role data only if it belongs to the current user's pos_accnt_id
            $lims_role_data = Roles::where('id', $id)
                              ->where('pos_accnt_id', Auth::user()->pos_accnt_id)
                              ->first();
            
            // If no matching role found, it means it's not in this user's tenant
            if(!$lims_role_data) {
                return redirect()->back()->with('not_permitted', 'Sorry! You cannot access data from another account');
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        $input = $request->all();
        $lims_role_data = Roles::where('id', $input['role_id'])
                          ->where('pos_accnt_id', Auth::user()->pos_accnt_id)
                          ->first();
        
        // If no matching role found, it means it's not in this user's tenant
        if(!$lims_role_data) {
            return redirect()->back()->with('not_permitted', 'Sorry! You cannot modify data from another account');
        }
    ...
    public function permission($id)
    {
        if(Auth::user()->role_id <= 2) {
            ...
            // If no matching role found, it means it's not in this user's tenant
            if(!$lims_role_data) {
                return redirect()->back()->with('not_permitted', 'Sorry! You cannot access permissions for data from another account');
            }
    ...
    public function setPermission(Request $request)
    {
        ...
        // First verify this role belongs to the current user's pos_accnt_id
        $role_record = Roles::where('id', $request['role_id'])
                      ->where('pos_accnt_id', Auth::user()->pos_accnt_id)
                      ->first();
                      
        if(!$role_record) {
            return redirect()->back()->with('not_permitted', 'Sorry! You cannot modify permissions for data from another account');
        }
        
        $role = Role::firstOrCreate(['id' => $request['role_id']]);

        // The rest of the permission settings remain the same
        // (All the long if-else blocks for individual permissions)
