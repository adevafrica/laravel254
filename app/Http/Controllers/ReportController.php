   ...
   public function productQuantityAlert()
{
    ...
    if($role->hasPermissionTo('product-qty-alert')){
        ...
        // Filter products by pos_accnt_id matching the logged-in admin
        $lims_product_data = Product::select('name','code', 'image', 'qty', 'alert_quantity')
            ->where('is_active', true)
            ->where('pos_accnt_id', $pos_accnt_id) // Multi-tenancy filter
    ...
    public function dailySaleObjectiveData(Request $request)
{
    ...
    // Add filter by pos_accnt_id to all queries
    ...
    if(!empty($lims_dso_alert_data))
    {
        foreach ($lims_dso_alert_data as $key => $dso_alert_data)
        {
            ...
            foreach (json_decode($dso_alert_data->product_info) as $index => $product_info) {
                if($index)
                    $product_info_string .= ', ';
                $product_info_string .= $product_info->name.' ['.$product_info->code.']';
            }
            $nestedData['product_info'] = $product_info_string;
    ...
    public function productExpiry()
{
    ...
    // Calculate the expiry date based on the general settings
    $date = date('Y-m-d', strtotime('+'.$general_settings_data["expiry_value"].' '.$general_settings_data["expiry_type"]));

    // Fetch product data filtered by pos_accnt_id (multi-tenancy)
    $lims_product_data = DB::table('products')
        ...
        ->where([
            ...
            ['products.pos_accnt_id', '=', $posAccntId],  // Filter by pos_accnt_id
            ['product_batches.pos_accnt_id', '=', $posAccntId] // Filter product_batches by pos_accnt_id
        ])
        ->select('products.name', 'products.code', 'products.image', 'product_batches.batch_no', 'product_batches.expired_date', 'product_batches.qty')
        ->get();
   ...
   public function dailyPurchase($year, $month)
{
    ...
    if($role->hasPermissionTo('daily-purchase')){
        ...
        while($start <= $number_of_day)
        {
            ...
            // Filter purchase data by pos_accnt_id
        ...
        // Pass pos_accnt_id to the view for additional filtering
    ...
    public function monthlySale($year)
{
    ...
    if($role->hasPermissionTo('monthly-sale')){
        ...
        // Pass pos_accnt_id to the view for further filtering if needed
        return view('backend.report.monthly_sale', compact('year', 'total_discount', 'order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'total', 'lims_warehouse_list', 'warehouse_id', 'pos_accnt_id'));
    }
    else
        return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
}
    ...
    public function monthlyPurchase($year)
{
    ...
    if($role->hasPermissionTo('monthly-purchase')){
        ...
        while($start <= $end)
        {
            ...
            // Filter purchases by pos_accnt_id
            $purchase_data = Purchase::whereDate('created_at', '>=' , $start_date)
                ->whereDate('created_at', '<=' , $end_date)
                ->where('pos_accnt_id', $pos_accnt_id) // Add this filter
                ->selectRaw(implode(',', $query1))
                ->get();
        ...
        // Filter warehouses by pos_accnt_id if needed
        $lims_warehouse_list = Warehouse::where('is_active', true)
            ->where('pos_accnt_id', $pos_accnt_id)
            ->get();
   ...
   public function monthlyPurchaseByWarehouse(Request $request, $year)
{
    ...
    while($start <= $end)
    {
        ...
        // Add pos_accnt_id filter to the purchase query
        $purchase_data = Purchase::where('warehouse_id', $data['warehouse_id'])
                        ->where('pos_accnt_id', $pos_accnt_id) // Add this line
    ...
    // Filter warehouses list by pos_accnt_id
    $lims_warehouse_list = Warehouse::where('is_active', true)
                          ->where('pos_accnt_id', $pos_accnt_id) // Add this line
                          ->get();
   ...
   public function bestSeller()
{
    ...
    if($role->hasPermissionTo('best-seller')){
        ...
        while($start <= $end)
        {
            ...
            // Join with sales table to filter by pos_accnt_id
            $best_selling_qty = Product_Sale::select(DB::raw('product_sales.product_id, sum(product_sales.qty) as sold_qty'))
            ...
            foreach ($best_selling_qty as $best_seller) {
                // Only get products that belong to the admin's account
                $product_data = Product::where('id', $best_seller->product_id)
                                      ->where('pos_accnt_id', $pos_accnt_id)
                                      ->first();
   ...
   public function profitLoss(Request $request)
{
    ...
    // Update all payment queries similarly
    ...
    $gift_card_payment_sale = DB::table('payments')
                        ->join('sales', 'payments.sale_id', '=', 'sales.id')
                        ->where('payments.paying_method', 'Gift Card')
                        ->whereNotNull('payments.sale_id')
                        ->where('sales.pos_accnt_id', $pos_accnt_id)
                        ->whereDate('payments.created_at', '>=' , $start_date)
                        ->whereDate('payments.created_at', '<=' , $end_date)
    ...
    $paypal_payment_sale = DB::table('payments')
                        ->join('sales', 'payments.sale_id', '=', 'sales.id')
                        ->where('payments.paying_method', 'Paypal')
                        ->whereNotNull('payments.sale_id')
                        ->where('sales.pos_accnt_id', $pos_accnt_id)
                        ->whereDate('payments.created_at', '>=' , $start_date)
                        ->whereDate('payments.created_at', '<=' , $end_date)
    ...
    $deposit_payment_sale = DB::table('payments')
                        ->join('sales', 'payments.sale_id', '=', 'sales.id')
                        ->where('payments.paying_method', 'Deposit')
                        ->whereNotNull('payments.sale_id')
                        ->where('sales.pos_accnt_id', $pos_accnt_id)
                        ->whereDate('payments.created_at', '>=' , $start_date)
                        ->whereDate('payments.created_at', '<=' , $end_date)
   ...
   public function purchaseReport(Request $request)
{
    ...
    // Only get products belonging to the current user's account
    ...
    foreach ($lims_product_all as $product) {
        ...
        if($warehouse_id == 0) {
            if($product->is_variant)
                $variant_id_all = ProductPurchase::distinct('variant_id')
                                  ->where('product_id', $product->id)
                                  ->where('pos_accnt_id', $pos_accnt_id) // Add tenant filter
                                  ->whereDate('created_at', '>=' , $start_date)
                                  ->whereDate('created_at', '<=' , $end_date)
                                  ->pluck('variant_id');
            else
                $lims_product_purchase_data = ProductPurchase::where('product_id', $product->id)
                                             ->where('pos_accnt_id', $pos_accnt_id) // Add tenant filter
                                             ->whereDate('created_at', '>=' , $start_date)
                                             ->whereDate('created_at', '<=' , $end_date)
                                             ->first();
            ...
            if($product->is_variant)
                $variant_id_all = DB::table('purchases')
                    ...
                    ->where([
                        ['product_purchases.product_id', $product->id],
                        ['purchases.warehouse_id', $warehouse_id],
                        ['purchases.pos_accnt_id', $pos_accnt_id], // Add tenant filter
                        ['product_purchases.pos_accnt_id', $pos_accnt_id] // Add tenant filter
        ...
        if($lims_product_purchase_data) {
            ...
            if($warehouse_id == 0)
                ...
                $product_qty[] = Product_Warehouse::where([
                                ['product_id', $product->id],
                                ['warehouse_id', $warehouse_id],
                                ['pos_accnt_id', $pos_accnt_id] // Add tenant filter
                            ])->sum('qty');
        }
        elseif(count($variant_id_all)) {
            foreach ($variant_id_all as $key => $variantId) {
                ...
                if($warehouse_id == 0)
                    $product_qty[] = ProductVariant::FindExactProduct($product->id, $variant_data->id)
                                    ->where('pos_accnt_id', $pos_accnt_id) // Add tenant filter
                                    ->first()->qty;
                else
                    $product_qty[] = Product_Warehouse::where([
    ...
    public function purchaseReportData(Request $request)
{
    ...
    if($request->input('search.value')) {
        ...
        $totalData = Product::where([
            ['name', 'LIKE', "%{$search}%"],
            ['is_active', true],
            ['pos_accnt_id', $admin_pos_accnt_id] // Filter by pos_accnt_id
        ])->count();
        $lims_product_all = Product::with('category')
                            ->select('id', 'name', 'code', 'category_id', 'qty', 'is_variant', 'price', 'cost')
                            ->where([
    ...
    foreach ($lims_product_all as $product) {
        ...
        if($warehouse_id == 0) {
            if($product->is_variant) {
                $variant_id_all = ProductVariant::where([
                    ['product_id', $product->id],
                    ['pos_accnt_id', $admin_pos_accnt_id] // Filter by pos_accnt_id
                ])->pluck('variant_id', 'item_code');
                ...
                foreach ($variant_id_all as $item_code => $variant_id) {
                    ...
                    if(count($lims_product_purchase_data)) {
                        foreach ($lims_product_purchase_data as $product_purchase) {
                            $unit = DB::table('units')->where([
                                ['id', $product_purchase->purchase_unit_id],
                                ['pos_accnt_id', $admin_pos_accnt_id] // Filter by pos_accnt_id
                            ])->first();
                ...
                foreach ($variant_id_all as $item_code => $variant_id) {
                    ...
                    $nestedData['purchased_amount'] = DB::table('purchases')
                                ...
                                ->where([
                                    ...
                                    ['product_purchases.variant_id', $variant_id],
                                    ['purchases.warehouse_id', $warehouse_id],
                                    ['purchases.pos_accnt_id', $admin_pos_accnt_id], // Filter by pos_accnt_id
                                    ['product_purchases.pos_accnt_id', $admin_pos_accnt_id] // Filter by pos_accnt_id
                                ])
                                ->whereDate('purchases.created_at','>=', $start_date)
                                ->whereDate('purchases.created_at','<=', $end_date)
                                ->sum('total');
    ...
    public function paymentReportByDate(Request $request)
{
    ...
    // Filter payments by date range AND pos_accnt_id
   ...
   public function customerReport(Request $request)
{
    ...
    // Get the customer_id from request, if provided
    $customer_id = $request->input('customer_id');
    
    // Set start and end date for the report, default to the past year if not provided
    if($request->input('start_date')) {
        $start_date = $request->input('start_date');
        $end_date = $request->input('end_date');
    }
    ...
    // Filter customers based on pos_accnt_id matching the logged-in admin's pos_accnt_id
    $lims_customer_list = Customer::where('is_active', true)
                                ->where('pos_accnt_id', $pos_accnt_id)  // Filtering by the logged-in user's pos_accnt_id
                                ->get();
    ...
    public function customerGroupReport(Request $request)
{
    ...
    // Modify the query to filter by pos_accnt_id
    $lims_customer_group_list = CustomerGroup::where('is_active', true)
        ->where('pos_accnt_id', $admin_pos_accnt_id)
        ->get();
    
    return view('backend.report.customer_group_report', compact(
        'starting_date', 
        'ending_date', 
        'customer_group_id', 
        'lims_customer_group_list',
        'admin_pos_accnt_id' // Pass this to the view if needed
    ));
}
   ...
   public function supplierReport(Request $request)
{
    ...
    // Filter suppliers by the admin's pos_accnt_id
    $lims_supplier_list = Supplier::where('is_active', true)
                                 ->where('pos_accnt_id', $pos_accnt_id)
                                 ->get();
    
    return view('backend.report.supplier_report', compact('start_date', 'end_date', 'supplier_id', 'lims_supplier_list', 'pos_accnt_id'));
}
    ...
    public function supplierPurchaseData(Request $request)
{
    ...
    $q = DB::table('purchases')
        ...
        ->where('purchases.pos_accnt_id', $pos_accnt_id) // Added tenant filtering
        ->whereDate('purchases.created_at', '>=' ,$request->input('start_date'))
        ->whereDate('purchases.created_at', '<=' ,$request->input('end_date'));
    ...
    else
    {
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $purchases =  $q->orwhere([
                            ['purchases.reference_no', 'LIKE', "%{$search}%"],
                            ['purchases.user_id', Auth::id()],
                            ['purchases.pos_accnt_id', $pos_accnt_id] // Added tenant filtering
                        ])
                        ->orwhere([
                            ['purchases.created_at', 'LIKE', "%{$search}%"],
                            ['purchases.user_id', Auth::id()],
    ...
    if(!empty($purchases))
    {
        foreach ($purchases as $key => $purchase)
        {
            ...
            // Add tenant filtering to product purchase data query
            ...
            foreach ($product_purchase_data as $index => $product_purchase) {
                if($product_purchase->purchase_unit_id) {
                    $unit_data = DB::table('units')
                                ->where('pos_accnt_id', $pos_accnt_id) // Added tenant filtering
                                ->select('unit_code')
                                ->find($product_purchase->purchase_unit_id);
