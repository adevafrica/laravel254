    ...
    public function index(Request $request)
{
    ...
    if($role->hasPermissionTo('purchases-index')) {
        // Get logged in user's pos_accnt_id for multi-tenancy
        ...
        $lims_pos_setting_data = PosSetting::select('stripe_public_key')->where('pos_accnt_id', $pos_accnt_id)->latest()->first();
        $lims_warehouse_list = Warehouse::where('is_active', true)->where('pos_accnt_id', $pos_accnt_id)->get();
        $lims_account_list = Account::where('is_active', true)->where('pos_accnt_id', $pos_accnt_id)->get();
...
public function purchaseData(Request $request)
{
    ...
    $q = Purchase::whereDate('created_at', '>=' ,$request->input('starting_date'))
                ...
                ->where('pos_accnt_id', $pos_accnt_id); // Apply multi-tenancy filter
    ...
    else
    {
        ...
        $q = Purchase::join('product_purchases', 'purchases.id', '=', 'product_purchases.purchase_id')
            ...
            ->where('purchases.pos_accnt_id', $pos_accnt_id) // Apply multi-tenancy filter
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $q =  $q->with('supplier', 'warehouse')
                    ...
                    ->orwhere([
                        ...
                        ['purchases.pos_accnt_id', $pos_accnt_id] // Apply multi-tenancy filter
                    ])
                    ->orwhere([
...
public function create()
{
    ...
    if($role->hasPermissionTo('purchases-add')){
        ...
        $lims_tax_list = Tax::where('is_active', true)
                          ->where('pos_accnt_id', $pos_accnt_id)
                          ->get();
        $lims_product_list_without_variant = $this->productWithoutVariant();
        $lims_product_list_with_variant = $this->productWithVariant();
        $currency_list = Currency::where('is_active', true)
                              ->where('pos_accnt_id', $pos_accnt_id)
                              ->get();
...
public function productWithoutVariant()
{
    ...
    return Product::ActiveStandard()->select('id', 'name', 'code')
            ...
            ->where('pos_accnt_id', $pos_accnt_id)
            ->whereHas('category', function ($query) use ($pos_accnt_id) {
...
public function productWithVariant()
{
    ...
    return Product::with(['productVariants' => function ($q) use ($pos_accnt_id) {
                $q->select('product_id', 'item_code')
                  ->where('pos_accnt_id', $pos_accnt_id)
                  ->orderBy('position');
            }])
            ...
            ->flatMap(function ($product) {
                return $product->productVariants->map(function ($variant) use ($product) {
                    return [
                        'id' => $product->id,
                        'name' => $product->name,
...
public function limsProductSearch(Request $request)
{
    ...
    if(!$lims_product_data) {
        ...
        $lims_product_data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
            ->where([
                ...
                ['products.pos_accnt_id', $pos_accnt_id],
                ['product_variants.pos_accnt_id', $pos_accnt_id]
            ])
            ->whereNotNull('is_variant')
    ...
    $units = Unit::where("base_unit", $lims_product_data->unit_id)
                ->where('pos_accnt_id', $pos_accnt_id)
                ->orWhere(function($query) use ($lims_product_data, $pos_accnt_id) {
                    $query->where('id', $lims_product_data->unit_id)
                          ->where('pos_accnt_id', $pos_accnt_id);
                })
                ->get();
    $unit_name = array();
 ...
 public function store(Request $request)
{
    ...
    foreach ($product_id as $i => $id) {
        $lims_purchase_unit_data = Unit::where('unit_name', $purchase_unit[$i])
            ->where('pos_accnt_id', Auth::user()->pos_accnt_id)
            ->first();

        $quantity = ($lims_purchase_unit_data->operator == '*')
            ? $recieved[$i] * $lims_purchase_unit_data->operation_value
            : $recieved[$i] / $lims_purchase_unit_data->operation_value;
        ...
        if ($lims_product_warehouse_data) {
            ...
            $lims_product_warehouse_data->qty = $quantity;
            $lims_product_warehouse_data->pos_accnt_id = Auth::user()->pos_accnt_id;
            if ($price)
        ...
        $product_purchase['pos_accnt_id'] = Auth::user()->pos_accnt_id;
        ProductPurchase::create($product_purchase);
    }

    // ✅ Update Expense Record by Adding to Existing Amount
    $defaultExpenseCategory = ExpenseCategory::where('name', 'Purchase')
        ->where('pos_accnt_id', Auth::user()->pos_accnt_id)
        ->first();

    if ($defaultExpenseCategory) {
        // Get the most recent expense record for this category and account
        $existingExpense = Expense::where('expense_category_id', $defaultExpenseCategory->id)
            ->where('pos_accnt_id', Auth::user()->pos_accnt_id)
            ->where('account_id', 1) // Match the same account
            ->orderBy('created_at', 'desc')
            ->first();

        $purchaseAmount = array_sum($data['subtotal']);

        if ($existingExpense) {
            // Update existing expense by adding the new purchase amount
            $existingExpense->amount += $purchaseAmount;
            $existingExpense->note = 'Accumulated Purchase expenses - Last updated: ' . date('Y-m-d H:i:s');
            $existingExpense->updated_at = now();
            $existingExpense->save();
        } else {
            // Create new expense record only if none exists
            Expense::create([
                'reference_no' => 'exp-' . date("Ymd") . '-' . date("his"),
                'amount' => $purchaseAmount,
                'note' => 'Initial Purchase expense entry',
...
{
    try {
        // Get the purchase record first to check if it belongs to this account
        $purchase = Purchase::where('id', $id)
                    ->where('pos_accnt_id', Auth::user()->pos_accnt_id)
                    ->first();
                    
        if (!$purchase) {
            return 'Purchase not found or access denied';
        }
        
        $lims_product_purchase_data = ProductPurchase::where('purchase_id', $id)
                                    ->where('pos_accnt_id', Auth::user()->pos_accnt_id) // Filter by pos_accnt_id
                                    ->get();
        $product_purchase = [];
        foreach ($lims_product_purchase_data as $key => $product_purchase_data) {
            $product = Product::where('id', $product_purchase_data->product_id)
                        ->where('pos_accnt_id', Auth::user()->pos_accnt_id) // Filter by pos_accnt_id
                        ->first();
            $unit = Unit::where('id', $product_purchase_data->purchase_unit_id)
                    ->where('pos_accnt_id', Auth::user()->pos_accnt_id) // Filter by pos_accnt_id
                    ->first();
            if($product_purchase_data->variant_id) {
                $lims_product_variant_data = ProductVariant::where('pos_accnt_id', Auth::user()->pos_accnt_id) // Filter by pos_accnt_id
                                            ->FindExactProduct($product->id, $product_purchase_data->variant_id)
                                            ->select('item_code')
                                            ->first();
                $product->code = $lims_product_variant_data->item_code;
            }
            if($product_purchase_data->product_batch_id) {
                $product_batch_data = ProductBatch::where('id', $product_purchase_data->product_batch_id)
                                    ->where('pos_accnt_id', Auth::user()->pos_accnt_id) // Filter by pos_accnt_id
                                    ->select('batch_no')
                                    ->first();
            ...
            if($product_purchase_data->imei_number) {
                ...
                ->where([
                    ...
                    ['return_purchases.pos_accnt_id', Auth::user()->pos_accnt_id], // Filter by pos_accnt_id
                    ['purchase_product_return.pos_accnt_id', Auth::user()->pos_accnt_id] // Filter by pos_accnt_id
    ...
    public function addPayment(Request $request)
{
    ...
    // Find the purchase that belongs to the current admin's account
    $lims_purchase_data = Purchase::where('pos_accnt_id', $admin_pos_accnt_id)
                             ->find($data['purchase_id']);
    
    // If the purchase doesn't belong to this admin, return with error
    if(!$lims_purchase_data) {
        return redirect('purchases')->with('error', 'Purchase not found or access denied');
    }
    ...
    // Add the pos_accnt_id to the payment record
    $lims_payment_data->pos_accnt_id = $admin_pos_accnt_id;
    $lims_payment_data->save();

    $lims_payment_data = Payment::where('pos_accnt_id', $admin_pos_accnt_id)
                           ->latest()->first();
    $data['payment_id'] = $lims_payment_data->id;
    
    // Get the PosSetting for this admin's account
    $lims_pos_setting_data = PosSetting::where('pos_accnt_id', $admin_pos_accnt_id)
                               ->latest()->first();
    ...
    if($paying_method == 'Credit Card' && $lims_pos_setting_data && $lims_pos_setting_data->stripe_secret_key){
        ...
        // Add pos_accnt_id to the data for PaymentWithCreditCard
        $data['pos_accnt_id'] = $admin_pos_accnt_id;
        PaymentWithCreditCard::create($data);
    }
    elseif ($paying_method == 'Cheque') {
        // Add pos_accnt_id to the data for PaymentWithCheque
        $data['pos_accnt_id'] = $admin_pos_accnt_id;
...
public function getPayment($id)
{
    ...
    // Get payments for the purchase that belong to this admin's account
    $lims_payment_list = Payment::where('purchase_id', $id)
    ...
    foreach ($lims_payment_list as $payment) {
        ...
        if($payment->paying_method == 'Cheque'){
            $lims_payment_cheque_data = PaymentWithCheque::where('payment_id', $payment->id)
                                         ->where('pos_accnt_id', $admin_pos_accnt_id)
                                         ->first();
        ...
        // Get the account for this admin's account
        $lims_account_data = Account::where('pos_accnt_id', $admin_pos_accnt_id)
