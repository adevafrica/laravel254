    ...
    public function generalSetting()
{
    ...
    // Query data filtered by pos_accnt_id
    $lims_general_setting_data = GeneralSetting::where('pos_accnt_id', $pos_accnt_id)->latest()->first();
    $lims_account_list = Account::where('pos_accnt_id', $pos_accnt_id)->where('is_active', true)->get();
    $lims_currency_list = Currency::where('pos_accnt_id', $pos_accnt_id)->get();
...
public function generalSettingStore(Request $request)
{
    ...
    // Find the general setting by pos_accnt_id or create a new one
    $general_setting = GeneralSetting::where('pos_accnt_id', $pos_accnt_id)->latest()->first();
    
    if (!$general_setting) {
        $general_setting = new GeneralSetting();
        $general_setting->pos_accnt_id = $pos_accnt_id; // Set the pos_accnt_id for new records
    }
    ...
    if ($logo) {
        // Only delete the logo if it belongs to this tenant
        if ($general_setting->site_logo) {
            $this->fileDelete('logo/', $general_setting->site_logo);
        }

        $ext = pathinfo($logo->getClientOriginalName(), PATHINFO_EXTENSION);
        $logoName = date("Ymdhis") . '_' . $pos_accnt_id . '.' . $ext; // Include pos_accnt_id in the filename
...
{
    // Get logged in admin's pos_accnt_id
    $pos_accnt_id = auth()->user()->pos_accnt_id;
    
    // Fetch only settings belonging to this admin
    $settings = ExternalService::where('pos_accnt_id', $pos_accnt_id)->get();
    $tonkra = [];
    $revesms = [];
    $bdbulksms = [];
    $twilio = [];
...
public function createSms()
{
    ...
    // Filter customer list by pos_accnt_id
    $lims_customer_list = Customer::where('is_active', true)
                                  ->where('pos_accnt_id', $pos_accnt_id)
                                  ->get();
    
    // Filter SMS templates by pos_accnt_id                             
    $smsTemplates = SmsTemplate::where('pos_accnt_id', $pos_accnt_id)->get();
...
public function processSmsData($templateId, $customerId, $referenceNo)
{
    ...
    $smsTemplate = SmsTemplate::where('id', $templateId)
                              ->where('pos_accnt_id', $pos_accnt_id)
                              ->first();
...
{
    // Get logged in user's pos_accnt_id
    $pos_accnt_id = auth()->user()->pos_accnt_id;
    
    // Fetch HRM setting data filtered by pos_accnt_id
    $lims_hrm_setting_data = HrmSetting::where('pos_accnt_id', $pos_accnt_id)
                                      ->latest()
                                      ->first();
...
public function hrmSettingStore(Request $request)
{
    ...
    // Find or create record specific to this pos_accnt_id
    $lims_hrm_setting_data = HrmSetting::firstOrNew([
        'pos_accnt_id' => $pos_accnt_id
    ]);
    
    $lims_hrm_setting_data->checkin = $data['checkin'];
    $lims_hrm_setting_data->checkout = $data['checkout'];
    $lims_hrm_setting_data->pos_accnt_id = $pos_accnt_id; // Ensure pos_accnt_id is set
...
{
    // Get current admin's pos_accnt_id
    $pos_accnt_id = auth()->user()->pos_accnt_id;
    ...
    // Get only the pos settings for this admin's account
    $lims_pos_setting_data = PosSetting::where('pos_accnt_id', $pos_accnt_id)
                                      ->latest()
                                      ->first();
...
public function posSettingStore(Request $request)
{
    ...
    // Find the pos setting for this specific account
    // Note: We're using firstOrNew but with pos_accnt_id as part of the criteria
    $pos_setting = PosSetting::firstOrNew([
        'pos_accnt_id' => $pos_accnt_id
    ]);
    
    // Ensure this setting belongs to the current admin
    $pos_setting->pos_accnt_id = $pos_accnt_id;
    
    // Make sure selected customer, warehouse and biller belong to this admin
    // You may want to add validation here to ensure the selected IDs belong to the current pos_accnt_id
    ...
    // Clear cache with specific key for this admin's pos_accnt_id
    cache()->forget('pos_setting_'.$pos_accnt_id);
