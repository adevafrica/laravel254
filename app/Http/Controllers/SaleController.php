...
public function store(Request $request)
{
    ...
    // Fetch the pos_accnt_id from the logged-in admin/user
    $loggedInUser = Auth::user();
    $data['pos_accnt_id'] = $loggedInUser->pos_accnt_id;
    ...
    // Ensure pos_accnt_id is added to the sales record
    ...
    if ($lims_reward_point_setting_data && $lims_reward_point_setting_data->is_active && $data['grand_total'] >= $lims_reward_point_setting_data->minimum_amount) {
        ...
        $lims_customer_data->LoyaltyPoints += $point;
    ...
    foreach ($product_id as $i => $id) {
        ...
        // IMPORTANT: Always add pos_accnt_id to product_sale data
        $product_sale['pos_accnt_id'] = $loggedInUser->pos_accnt_id;
        ...
        // Create product_sale record with pos_accnt_id
    ...
    // Variable to store the payment method
    ...
    if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
        foreach ($data['paid_by_id'] as $key => $value) {
            if ($data['paid_amount'][$key] > 0) {
                ...
                // Only store the payment method if it's Cash or Mpesa
                if ($paying_method == 'Cash' || $paying_method == 'Mpesa') {
                    $primary_payment_method = $paying_method;
                }
                ...
                // IMPORTANT: Always add pos_accnt_id to payment data
                $lims_payment_data->pos_accnt_id = $loggedInUser->pos_accnt_id;
                ...
                $lims_pos_setting_data = PosSetting::latest()->first();
                // Limit payment methods to Cash and Mpesa, with special handling for each
                if ($paying_method == 'Cash') {
                    // No special handling needed for cash payments
                } elseif ($paying_method == 'Mpesa') {
                    // Capture Mpesa transaction details if available
                    if (isset($data['mpesa_transaction_id'])) {
                        // You can add code here to store Mpesa transaction details
                        // For example, create a new model for Mpesa payments
                    }
                    ...
                    $lims_customer_data->LoyaltyPoints -= $data['used_points'];
                    $lims_customer_data->save();
        ...
        // Update the sales record with the payment method
        if (!empty($primary_payment_method)) {
            Sale::where('id', $lims_sale_data->id)->update(['payment_method' => $primary_payment_method]);
        }

        // Calculate total profit/points for the current customer only, filtered by pos_accnt_id
        $customerTotalPoints = DB::table('sales')
            ->join('product_sales', 'sales.id', '=', 'product_sales.sale_id')
            ->join('products', 'product_sales.product_id', '=', 'products.id')
            ->where('sales.customer_id', $data['customer_id'])
            ->where('product_sales.pos_accnt_id', $loggedInUser->pos_accnt_id) // Filter by pos_accnt_id - IMPORTANT
            ->select(
                DB::raw('SUM((products.price * product_sales.qty) - (products.cost * product_sales.qty)) as total_profit')
            )
            ->first();

        // Get the total profit/points value
        $totalPoints = $customerTotalPoints->total_profit ?? 0;

        // Also update the customer data object we're already using
        $lims_customer_data->LoyaltyPoints = $totalPoints;
        
        // NEW CODE: Update the total_inpl column in customers table
        $lims_customer_data->total_inpl = $totalPoints;
        
        $lims_customer_data->save();

        // =====================================================
// UPDATED FEATURE: Update Account Balances Based on Percentage of PROFIT
// =====================================================
if ($lims_sale_data->sale_status == 1) { // Only process for completed sales
    
    try {
        // Get all accounts that have percentage values and are active
        $accountsWithPercentage = Account::where('is_active', 1)
            ->whereNotNull('percentage')
            ->where('percentage', '>', 0)
            ->when($loggedInUser->pos_accnt_id, function($query) use ($loggedInUser) {
                return $query->where('pos_accnt_id', $loggedInUser->pos_accnt_id);
            })
            ->get();
        
        if ($accountsWithPercentage->count() > 0) {
            
            // Calculate profit for this specific sale (filtered by pos_accnt_id)
            $saleProfit = DB::table('product_sales')
                ->join('products', 'product_sales.product_id', '=', 'products.id')
                ->where('product_sales.sale_id', $lims_sale_data->id)
                ->where('product_sales.pos_accnt_id', $loggedInUser->pos_accnt_id) // Filter by pos_accnt_id
                ->select(
                    DB::raw('SUM((products.price * product_sales.qty) - (products.cost * product_sales.qty)) as total_profit')
                )
                ->first();
            
            // Get the profit value (use 0 if null or negative)
            $profitAmount = max(0, $saleProfit->total_profit ?? 0);
            
            // Only proceed if there's actual profit
            if ($profitAmount > 0) {
                $updatedAccountsCount = 0;
                
                foreach ($accountsWithPercentage as $account) {
                    
                    // Calculate the percentage amount from the PROFIT (not sale total)
                    $percentageAmount = ($account->percentage / 100) * $profitAmount;
                    
                    // Get the current balance (default to 0 if null)
                    $currentBalance = $account->balance ?? 0;
                    
                    // Add the percentage amount to the existing balance
                    $newBalance = $currentBalance + $percentageAmount;
                    ...
                    // Optional: Create audit log for the percentage allocation
                    try {
                        DB::table('account_percentage_logs')->insert([
                            'account_id' => $account->id,
                            'sale_id' => $lims_sale_data->id,
                            'account_name' => $account->name ?? '',
                            'percentage_applied' => $account->percentage,
                            'sale_total' => $lims_sale_data->grand_total, // Keep sale total for reference
                            'profit_amount' => $profitAmount, // NEW: Log the profit amount
                            'percentage_amount' => $percentageAmount,
                            'previous_balance' => $currentBalance,
                            'new_balance' => $newBalance,
                            'reference_no' => $lims_sale_data->reference_no,
                            'pos_accnt_id' => $loggedInUser->pos_accnt_id,
                            'created_at' => now(),
                            'updated_at' => now()
                        ]);
                    } catch (\Exception $logException) {
                        // If logging fails, continue with the main process
                        \Log::warning("Failed to log percentage allocation for account {$account->id}: " . $logException->getMessage());
                    }
                }
                
                // Log success message
                \Log::info("Successfully updated {$updatedAccountsCount} account balances based on {$profitAmount} profit for sale ID: {$lims_sale_data->id} (Reference: {$lims_sale_data->reference_no})");
                
                // Add to success message
                $message .= " and updated {$updatedAccountsCount} account balances based on profit";
            } else {
                // Log when no profit was made
                \Log::info("No profit generated for sale ID: {$lims_sale_data->id}, skipping percentage allocation");
            }
        }
        
    } catch (\Exception $e) {
        // Log the error but don't fail the entire sale process
        \Log::error("Error updating account balances for sale {$lims_sale_data->id}: " . $e->getMessage());
        
        // Optionally, you can add error handling here
        // For now, we'll continue with the sale process
    }
}
// =====================================================
// END: Account Balance Update Feature (Based on Profit)
// =====================================================

        $mail_data["name"] = $lims_customer_data->name;
        $mail_data["member_no"] = $lims_customer_data->MemberNo;
        $mail_data['points'] = $lims_customer_data->LoyaltyPoints ?? 0;
        $mail_data["inpl"] = $lims_customer_data->INPL + $mail_data['points'];
        if ($mail_data["inpl"] > 0) {
            $mail_data["inpl"] = 0;
        }

        // IMPORTANT: Filter tax rates by pos_accnt_id for multi-tenancy
        $taxRates = DB::table('taxes')
            ...
            ->get();
        $mail_data['tax_rates'] = $taxRates;

        // IMPORTANT: Filter active accounts by pos_accnt_id for multi-tenancy
        $activeAccount = DB::table('accounts')
            ->where('is_active', 1)
            ->when($loggedInUser->pos_accnt_id, function($query) use ($loggedInUser) {
                return $query->where('pos_accnt_id', $loggedInUser->pos_accnt_id);
            })
            ...
            ->first();
        $mail_data['active_account'] = $activeAccount;

        // Add phone number for loyalty slip
    ...
    // IMPORTANT: Filter loyalty account by pos_accnt_id for multi-tenancy
    $LoyaltyAccount = Account::where('name', 'like', '%LOYALTY ACCOUNT%')
        ->when($loggedInUser->pos_accnt_id, function($query) use ($loggedInUser) {
            return $query->where('pos_accnt_id', $loggedInUser->pos_accnt_id);
        })
        ->first();
    ...
    if ($LoyaltyAccount) {
        // IMPORTANT: Filter profit calculation by pos_accnt_id for multi-tenancy
        $latestProfit = DB::table('sales')
            ->select(
                'sales.id',
                DB::raw('SUM((products.price - products.cost) * product_sales.qty) as profit'),
                'sales.created_at'
            )
        ...
        $totalProfit = $latestProfit ? $latestProfit->profit : 0;

        //$LoyaltyAccount->total_balance += $totalProfit;
        $LoyaltyAccount->initial_balance += $totalProfit;
        $LoyaltyAccount->save();
    }
    ...
    public function storeOld(Request $request)
    {
        ...
        try {*/
            ...
            $data['user_id'] = Auth::id();
            // Get the logged in user data to access pos_accnt_id
            ...
            foreach ($product_id as $i => $id) {
                ...
                $product_sale['tax'] = $tax[$i];
                $product_sale['total'] = $mail_data['total'][$i] = $total[$i];
                // Add pos_accnt_id from the logged-in user
                $product_sale['pos_accnt_id'] = $logged_in_user->pos_accnt_id;
                Product_Sale::create($product_sale);
            }
            if($data['sale_status'] == 3)
                $message = 'Sale successfully added to draft';
                ...
                foreach($data['paid_by_id'] as $key=>$value)
                {
                    if($data['paid_amount'][$key] > 0) {
                        ...
                        elseif($data['paid_by_id'][$key] == 8) {
                            ...
                            $paying_method = $data['paid_by_id'][$key]; 
                        }
    ...
    // Find the category id of 'Raw Materials'
    $rawMaterialsCategory = \DB::table('categories')->where('name', 'Raw Materials')->first();
    $rawMaterialsCategoryId = $rawMaterialsCategory ? $rawMaterialsCategory->id : null;

    // Start building the query
    $query = Product::join('product_warehouse', 'products.id', '=', 'product_warehouse.product_id')
                    ->where('products.pos_accnt_id', '=', $posAccntId);

    // Exclude products with 'Raw Materials' category if found
    if ($rawMaterialsCategoryId) {
        $query = $query->where('products.category_id', '!=', $rawMaterialsCategoryId);
    }
    ...
    // Change MySQL strict mode temporarily
    ...
    $lims_product_with_imei_warehouse_data = Product::join('product_warehouse', 'products.id', '=', 'product_warehouse.product_id')
        ->where([
            ...
            ['product_warehouse.warehouse_id', $id],
            ['product_warehouse.qty', '>', 0],
            ['products.pos_accnt_id', '=', $posAccntId]
        ])
        // Also exclude 'Raw Materials' products here too
        ->when($rawMaterialsCategoryId, function ($query) use ($rawMaterialsCategoryId) {
            return $query->where('products.category_id', '!=', $rawMaterialsCategoryId);
        })
        ->whereNull('product_warehouse.variant_id')
    ...
    public function posSale($id = '')
{
    ...
    if ($role->hasPermissionTo('sales-add')) {
        ...
        // Fetch customers based on the logged-in user's pos_accnt_id
        $lims_customer_list = Cache::remember('customer_list_' . $posAccntId, 60*60*24, function () use ($posAccntId) {
            return Customer::where('is_active', true)
                        ->where('pos_accnt_id', $posAccntId) // Filter based on pos_accnt_id
                        ->get();
        });

        // Fetch customer groups (no pos_accnt_id filter needed)
        ...
        // Fetch warehouses based on the logged-in user's pos_accnt_id
        $lims_warehouse_list = Cache::remember('warehouse_list_' . $posAccntId, 60*60*24*365, function () use ($posAccntId) {
            return Warehouse::where('is_active', true)
                            ->where('pos_accnt_id', $posAccntId) // Filter based on pos_accnt_id
                            ->get();
        });

        // Fetch billers based on the logged-in user's pos_accnt_id
        $lims_biller_list = Cache::remember('biller_list_' . $posAccntId, 60*60*24*30, function () use ($posAccntId) {
            return Biller::where('is_active', true)
                        ->where('pos_accnt_id', $posAccntId) // Filter based on pos_accnt_id
                        ->get();
        });

        // Fetch reward point settings
        $lims_reward_point_setting_data = RewardPointSetting::latest()->first();

        // Fetch tax list (no pos_accnt_id filtering here)
        ...
        // Fetch brands based on the logged-in user's pos_accnt_id
        $lims_brand_list = Cache::remember('brand_list_' . $posAccntId, 60*60*24*30, function () use ($posAccntId) {
            return Brand::where('is_active', true)
                        ->where('pos_accnt_id', $posAccntId) // Filter based on pos_accnt_id
                        ->get();
        });

        // Fetch categories based on the logged-in user's pos_accnt_id
        $lims_category_list = Cache::remember('category_list_' . $posAccntId, 60*60*24*30, function () use ($posAccntId) {
        ...
        // Filter numberOfInvoice count based on pos_accnt_id
        $numberOfInvoice = Sale::where('pos_accnt_id', $posAccntId)->count();
        
        $custom_fields = CustomField::where('belongs_to', 'sale')->get();

        if (isset($id)) {
            // Get sale data without pos_accnt_id filtering
            $lims_sale_data = Sale::find($id);
            
            // Filter product_sale data based on both sale_id and pos_accnt_id
            $lims_product_sale_data = Product_Sale::where('sale_id', $id)
                                      ->where('pos_accnt_id', $posAccntId)
                                      ->get();
            
            return view('backend.sale.pos', compact(
                'lims_sale_data', 'lims_product_sale_data', 'currency_list', 'role', 'all_permission', 
                'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 
                'lims_reward_point_setting_data', 'lims_tax_list', 'lims_biller_list', 
                'lims_pos_setting_data', 'options', 'lims_brand_list', 'lims_category_list', 
                'lims_table_list', 'lims_coupon_list', 'flag', 'numberOfInvoice', 'custom_fields'
            ));
        }

        return view('backend.sale.pos', compact(
            'currency_list', 'role', 'all_permission', 'lims_customer_list', 
            'lims_customer_group_all', 'lims_warehouse_list', 'lims_reward_point_setting_data', 
            'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'options', 
            'lims_brand_list', 'lims_category_list', 'lims_table_list', 'lims_coupon_list', 
            'flag', 'numberOfInvoice', 'custom_fields'
        ));
    } else {
        return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
       ...
       public function recentSale()
{
    ...
    if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
        // Fetch the recent sales for the logged-in admin's account
        $recent_sale = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')
            ...
            ->where([
                ['sales.sale_status', 1],
                ['sales.user_id', Auth::id()],
                ['customers.pos_accnt_id', $posAccntId] // Filter based on pos_accnt_id
            ])
        ...
        // Fetch the recent sales for all customers with the matching pos_accnt_id for the logged-in admin
        $recent_sale = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')
            ->select('sales.id', 'sales.reference_no', 'sales.customer_id', 'sales.grand_total', 'sales.created_at', 'customers.name')
            ->where([
                ['sales.sale_status', 1],
...
public function recentDraft()
{
    ...
    if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
        // Fetch recent drafts for the specific admin's pos_accnt_id
        $recent_draft = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')
            ->select('sales.id', 'sales.reference_no', 'sales.customer_id', 'sales.grand_total', 'sales.created_at', 'customers.name')
            ->where([
                ['sales.sale_status', 3],
                ['sales.user_id', Auth::id()],
                ['customers.pos_accnt_id', $posAccntId]  // Filter by logged-in admin's pos_accnt_id
            ])
            ->orderBy('sales.id', 'desc')
            ->take(10)
            ->get();
        ...
        // Fetch all recent drafts for the logged-in admin's pos_accnt_id
        $recent_draft = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')
            ->select('sales.id', 'sales.reference_no', 'sales.customer_id', 'sales.grand_total', 'sales.created_at', 'customers.name')
            ->where([
                ['sales.sale_status', 3],
     ...
     public function getFeatured(Request $request)
{
    ...
    // Modify the query to filter products by the logged-in admin's pos_accnt_id
    $lims_product_list = Product::where([
        ['is_active', true],
        ['featured', true],
...
{
    // Get the currently logged in user's pos_accnt_id
    $user_pos_accnt_id = auth()->user()->pos_accnt_id;
    
    // Filter product sales by both sale_id and pos_accnt_id
    $lims_product_sale_data = Product_Sale::where('sale_id', $id)
                                        ->where('pos_accnt_id', $user_pos_accnt_id)
                                        ->get();
    ...
    public function todaySale()
{
    ...
    $data['total_payment'] = Payment::whereDate('created_at', date("Y-m-d"))
                                    ->where('pos_accnt_id', $pos_accnt_id)
                                    ->sum('amount');
    ...
    $data['total_sale_return'] = Returns::whereDate('created_at', date("Y-m-d"))
                                        ->where('pos_accnt_id', $pos_accnt_id)
                                        ->sum('grand_total');
        ...
        if($warehouse_id == 0) {
            if($product_sale->product_batch_id)
                $product_purchase_data = ProductPurchase::where([
                    ...
                    ['pos_accnt_id', $pos_accnt_id] // Filter by pos_accnt_id
                ])->get();
            else
            ...
            if($product_sale->product_batch_id) {
                ...
                ->where([
                    ...
                    ['purchases.pos_accnt_id', $pos_accnt_id] // Filter by pos_accnt_id
