...
public function index()
{
    ...
    // Check if the user has permission to view the billers
    if($role->hasPermissionTo('billers-index')) {
        ...
        // Fetch billers where pos_accnt_id matches the logged-in admin's pos_accnt_id
        $lims_biller_all = biller::where('is_active', true)
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
         ...
         // Fetch logged-in admin's pos_accnt_id
    $posAccntId = Auth::user()->pos_accnt_id;

    // Prepare biller data (excluding image)
    $lims_biller_data = $request->except('image');
    $lims_biller_data['is_active'] = true;
    $lims_biller_data['pos_accnt_id'] = $posAccntId;  // Assign logged-in admin's pos_accnt_id to new biller
