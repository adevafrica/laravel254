   ...
   public function index(Request $request)
{
    ...
    // Check if the logged-in user has permission to view products
    if ($role->hasPermissionTo('products-index')) {
        
        // Get the logged-in user's pos_accnt_id
        $posAccntId = Auth::user()->pos_accnt_id;

        // Fetch the list of active warehouses
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();

        // Get the warehouse_id from the request or default to 0
        $warehouse_id = $request->input('warehouse_id', 0);

        // Fetch the permissions of the role
        ...
        // Count the total number of active products filtered by pos_accnt_id
        $numberOfProduct = DB::table('products')
            ->where('is_active', true)
            ->where('pos_accnt_id', $posAccntId) // Filter by pos_accnt_id
            ->count();

        // Fetch custom fields for products
        $custom_fields = CustomField::where([['belongs_to', 'product'], ['is_table', true]])->pluck('name');
        $field_name = [];
        foreach ($custom_fields as $fieldName) {
            $field_name[] = str_replace(" ", "_", strtolower($fieldName));
        }

        // Fetch products filtered by pos_accnt_id (only the logged-in admin's products)
...
$totalData = DB::table('products')->where('is_active', true)
                                      ->where('pos_accnt_id', $posAccntId) // Filter by pos_accnt_id
                                      ->count();        
        ...
        if(empty($request->input('search.value'))){
            // Fetch products based on pos_accnt_id of the logged-in user
        ...
        $q = Product::select('products.*')
                    ...
                    ->where('products.pos_accnt_id', $posAccntId) // Filter by pos_accnt_id
    ...
    public function create()
{
    ...
    if ($role->hasPermissionTo('products-add')){
        ...
        // Modify all queries to filter by pos_accnt_id
        $lims_product_list_without_variant = $this->productWithoutVariant()->where('pos_accnt_id', $pos_accnt_id);
        $lims_product_list_with_variant = $this->productWithVariant()->where('pos_accnt_id', $pos_accnt_id);
        ...
        $lims_warehouse_list = Warehouse::where('is_active', true)
                                ->where('pos_accnt_id', $pos_accnt_id)
                                ->get();
        $numberOfProduct = Product::where('is_active', true)
                              ->where('pos_accnt_id', $pos_accnt_id)
                              ->count();
        $custom_fields = CustomField::where('belongs_to', 'product')
                            ->where('pos_accnt_id', $pos_accnt_id)
                            ->get();
    ...
    $posAccntId = Auth::user()->pos_accnt_id;  //  the admin is logged in
        ...
        // Add the pos_accnt_id to the product data
    $data['pos_accnt_id'] = $posAccntId;  
    ...
    public function edit($id)
{
    ...
    if ($role->hasPermissionTo('products-edit')) {
        ...
        // Only fetch data matching the admin's pos_accnt_id
        $lims_product_list_without_variant = $this->productWithoutVariant()->where('pos_accnt_id', $pos_accnt_id);
        $lims_product_list_with_variant = $this->productWithVariant()->where('pos_accnt_id', $pos_accnt_id);
        $lims_brand_list = Brand::where('is_active', true)->where('pos_accnt_id', $pos_accnt_id)->get();
        $lims_category_list = Category::where('is_active', true)->where('pos_accnt_id', $pos_accnt_id)->get();
        $lims_unit_list = Unit::where('is_active', true)->where('pos_accnt_id', $pos_accnt_id)->get();
        $lims_tax_list = Tax::where('is_active', true)->where('pos_accnt_id', $pos_accnt_id)->get();
        
        // Ensure the product belongs to the admin's account
        $lims_product_data = Product::where('id', $id)
                                   ->where('pos_accnt_id', $pos_accnt_id)
                                   ->first();
        
        // If no product is found with matching pos_accnt_id, redirect with a message
        if(!$lims_product_data) {
            return redirect()->back()->with('not_permitted', 'Sorry! You do not have access to this product');
        }
        ...
        // Get only variants that belong to this product (variants should have product_id as foreign key)
        $lims_product_variant_data = $lims_product_data->variant()->orderBy('position')->get();
        $lims_warehouse_list = Warehouse::where('is_active', true)->where('pos_accnt_id', $pos_accnt_id)->get();
        $noOfVariantValue = 0;
        $custom_fields = CustomField::where('belongs_to', 'product')->where('pos_accnt_id', $pos_accnt_id)->get();
