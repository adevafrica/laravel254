    ...
    public function quotationData(Request $request)
{
    ...
    $warehouse_id = $request->input('warehouse_id');
    $posAccntId = Auth::user()->pos_accnt_id;  // Get logged-in user's pos_accnt_id
    
    // Calculate total data with pos_accnt_id filter
    if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
    ...
    elseif($warehouse_id != 0)
        ...
        $totalData = Quotation::where('pos_accnt_id', $posAccntId)
                    ->whereDate('created_at', '>=' ,$request->input('starting_date'))
                    ->whereDate('created_at', '<=' ,$request->input('ending_date'))
                    ->count();
    ...
    if(empty($request->input('search.value'))) {
        // Handle queries when there's no search term
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
            $quotations = Quotation::with('biller', 'customer', 'supplier', 'user', 'warehouse')
                        ->offset($start)
        ...
        // Handle queries when there's a search term
        ...
        if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $quotations = Quotation::select('quotations.*')
                ...
                ->where('quotations.pos_accnt_id', $posAccntId)
                ->where(function($query) use ($search) {
                    $query->whereDate('quotations.created_at', '=', date('Y-m-d', strtotime(str_replace('/', '-', $search))))
                        ->orWhere('quotations.reference_no', 'LIKE', "%{$search}%");
                })
                ->orWhere(function($query) use ($search, $posAccntId) {
                    $query->where('billers.name', 'LIKE', "%{$search}%")
                          ->where('billers.pos_accnt_id', $posAccntId);
                })
                ->orWhere(function($query) use ($search, $posAccntId) {
                    $query->where('customers.name', 'LIKE', "%{$search}%")
                          ->where('customers.pos_accnt_id', $posAccntId);
                })
                ->orWhere(function($query) use ($search, $posAccntId) {
                    $query->where('suppliers.name', 'LIKE', "%{$search}%")
                          ->whereNotNull('suppliers.pos_accnt_id')
                          ->where('suppliers.pos_accnt_id', $posAccntId);
                })
        ...
        elseif (Auth::user()->role_id > 2 && config('staff_access') == 'warehouse') {
            $quotations = Quotation::select('quotations.*')
                ...
                ->where('quotations.warehouse_id', Auth::user()->warehouse_id)
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            $quotations = Quotation::select('quotations.*')
                ...
                ->when($warehouse_id != 0, function ($query) use ($warehouse_id) {
                    return $query->where('quotations.warehouse_id', $warehouse_id);
                })
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
    ...
    if(!empty($quotations)) {
        foreach ($quotations as $key => $quotation) {
            // Process and build the response data
    ...
    public function create()
{
    ...
    if($role->hasPermissionTo('quotes-add')){
        ...
        // Filter data based on pos_accnt_id
    ...
    // Assign pos_accnt_id to the data array
    $data['user_id'] = Auth::id();
    $data['pos_accnt_id'] = $posAccntId; // Set pos_accnt_id for the quotation
   ...
   public function getCustomerGroup($id)
{
    ...
    // Fetch the customer by ID, but ensure the pos_accnt_id matches the logged-in admin's pos_accnt_id
    $lims_customer_data = Customer::where('id', $id)
                                  ->where('pos_accnt_id', $posAccntId)
                                  ->first();

    // If no customer is found, return null or handle accordingly
    if (!$lims_customer_data) {
        return null; // or any error handling logic you want
    }

    // Fetch the customer group associated with the customer, ensuring it also matches the pos_accnt_id
    $lims_customer_group_data = CustomerGroup::where('id', $lims_customer_data->customer_group_id)
                                            ->where('pos_accnt_id', $posAccntId)
                                            ->first();

    // If no customer group is found, return null or handle accordingly
    if (!$lims_customer_group_data) {
        return null; // or any error handling logic you want
    }

    // Return the percentage of the customer group
