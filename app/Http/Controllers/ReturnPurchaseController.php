    ...
    public function returnData(Request $request)
{
    ...
    // Base query condition for multi-tenancy
    $baseQuery = function($query) use ($admin_pos_accnt_id) {
        return $query->where('return_purchases.pos_accnt_id', $admin_pos_accnt_id);
    };

    // Count total data with date filter and multi-tenancy
    ...
    elseif($warehouse_id != 0)
        ...
        $totalData = ReturnPurchase::where('pos_accnt_id', $admin_pos_accnt_id)
                    ->whereDate('created_at', '>=' ,$request->input('starting_date'))
                    ->whereDate('created_at', '<=' ,$request->input('ending_date'))
                    ->count();
    ...
    if(empty($request->input('search.value'))) {
        $q = ReturnPurchase::with('supplier', 'warehouse', 'user')
            ->where('pos_accnt_id', $admin_pos_accnt_id) // Apply multi-tenancy filter
            ->whereDate('created_at', '>=' ,$request->input('starting_date'))
            ->whereDate('created_at', '<=' ,$request->input('ending_date'))
            ->offset($start)
            ->limit($limit)
        ...
        $q = ReturnPurchase::leftJoin('suppliers', 'return_purchases.supplier_id', '=', 'suppliers.id')
            ->where('return_purchases.pos_accnt_id', $admin_pos_accnt_id) // Apply multi-tenancy filter
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $returnss = $q->select('return_purchases.*')
                        ...
                        ->orWhere(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('return_purchases.reference_no', 'LIKE', "%{$search}%")
                                  ->where('return_purchases.user_id', Auth::id())
                                  ->where('return_purchases.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orWhere(function($query) use ($search, $admin_pos_accnt_id) {
            ...
            $totalFiltered = $q->where(function($query) use ($search, $admin_pos_accnt_id) {
        ...
        elseif(Auth::user()->role_id > 2 && config('staff_access') == 'warehouse') {
            $returnss = $q->select('return_purchases.*')
                        ...
                        ->where(function($query) use ($admin_pos_accnt_id) {
                            $query->where('return_purchases.warehouse_id', Auth::user()->warehouse_id)
                                  ->where('return_purchases.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orWhere(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('return_purchases.reference_no', 'LIKE', "%{$search}%")
                                  ->where('return_purchases.warehouse_id', Auth::user()->warehouse_id)
                                  ->where('return_purchases.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orWhere(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('suppliers.name', 'LIKE', "%{$search}%")
            ...
            $totalFiltered = $q->where(function($query) use ($admin_pos_accnt_id) {
                            $query->where('return_purchases.warehouse_id', Auth::user()->warehouse_id)
                                  ->where('return_purchases.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orWhere(function($query) use ($search, $admin_pos_accnt_id) {
            ...
            $returnss = $q->select('return_purchases.*')
                        ->with('supplier', 'warehouse', 'user')
                        ->where(function($query) use ($search, $admin_pos_accnt_id) {
                            $query->where('return_purchases.reference_no', 'LIKE', "%{$search}%")
                                  ->where('return_purchases.pos_accnt_id', $admin_pos_accnt_id);
                        })
                        ->orWhere(function($query) use ($search, $admin_pos_accnt_id) {
